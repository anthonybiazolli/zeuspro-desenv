...
import { LayoutDashboard, MessageCircle, Settings, LogOut, Zap, Send, UserPlus, X } from 'lucide-react';
...
export default function ChatPage() {
  ...
  const handleCreateNewChat = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('http://localhost:3000/whatsapp/contact', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: newChatName, phoneNumber: newChatPhone })
      });
      const newContact = await res.json();
      
      // Fecha o modal, limpa os campos e já seleciona a nova pessoa!
    ...
    <div className="flex h-screen bg-gray-100 text-gray-900 font-sans relative">
      ...
      {showNewChatModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full relative animate-in fade-in zoom-in duration-200">
            <button onClick={() => setShowNewChatModal(false)} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600 transition">
              <X className="w-6 h-6" />
            </button>
            <h3 className="text-2xl font-bold text-gray-900 mb-2">Novo Chat</h3>
            <p className="text-gray-500 mb-6 text-sm">Digite os dados do lead para iniciar uma conversa no WhatsApp.</p>
            ...
            <form onSubmit={handleCreateNewChat} className="space-y-4">
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent transition"
                />
              </div>
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent transition"
                />
                <p className="text-[11px] text-gray-400 mt-1">Insira o código do país (55) + DDD + Número. Apenas números.</p>
              </div>
              <button 
                type="submit" disabled={isCreatingChat}
                className="w-full bg-brand-500 hover:bg-brand-600 text-white font-semibold py-3 rounded-xl transition mt-4 flex items-center justify-center"
              >
      ...
      <aside className="w-20 bg-white border-r border-gray-200 flex flex-col items-center py-6 z-30 shadow-sm">
        <div className="w-10 h-10 bg-brand-500 rounded-xl flex items-center justify-center text-white mb-8 shadow-lg shadow-brand-500/30">
          <Zap className="w-6 h-6" />
        </div>
        <nav className="flex-1 space-y-4">
          <Link href="/dashboard" className="w-12 h-12 flex items-center justify-center rounded-xl text-gray-400 hover:bg-gray-50 hover:text-brand-600 transition" title="Dashboard">
            <LayoutDashboard className="w-6 h-6" />
          </Link>
          <div className="w-12 h-12 flex items-center justify-center rounded-xl bg-brand-50 text-brand-600 shadow-sm border border-brand-100" title="Chat IA">
            <MessageCircle className="w-6 h-6" />
          </div>
          <Link href="/dashboard/settings" className="w-12 h-12 flex items-center justify-center rounded-xl text-gray-400 hover:bg-gray-50 hover:text-brand-600 transition" title="Ajustes">
            <Settings className="w-6 h-6" />
          </Link>
        </nav>
        <button onClick={handleLogout} className="w-12 h-12 flex items-center justify-center rounded-xl text-gray-400 hover:text-red-500 hover:bg-red-50 transition" title="Sair">
      ...
      <div className="w-80 bg-white border-r border-gray-200 flex flex-col shadow-sm relative z-20">
        <div className="p-5 border-b border-gray-100 bg-gray-50/80 flex items-center justify-between">
          ...
          <button 
            ...
            className="w-10 h-10 bg-gray-100 hover:bg-brand-50 hover:text-brand-600 text-gray-600 rounded-full flex items-center justify-center transition"
            title="Nova Conversa"
          >
        ...
        <div className="flex-1 overflow-y-auto">
          {contacts.map(c => (
            <div 
              ...
              className={`flex items-center p-4 border-b border-gray-50 cursor-pointer transition-all duration-200 ${selectedContact?.id === c.id ? 'bg-brand-50/50 border-l-4 border-brand-500' : 'hover:bg-gray-50 border-l-4 border-transparent'}`}
            >
      ...
      <div className="flex-1 flex flex-col bg-[#efeae2] relative bg-[url('https://i.pinimg.com/1200x/8c/98/99/8c98994518b575bfd8c949e91d20548b.jpg')] bg-repeat opacity-95">
        ...
        {selectedContact ? (
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 z-10 scroll-smooth">
              {messages.length === 0 ? (
                 <div className="flex items-center justify-center h-full">
                    <div className="bg-white/90 backdrop-blur-sm px-6 py-3 rounded-2xl shadow-sm text-sm text-gray-500 font-medium">
                       As mensagens aparecerão aqui. Envie a primeira mensagem!
                    </div>
                 </div>
              ) : (
                messages.map((msg, idx) => (
                  <div key={msg.id || idx} className={`flex ${msg.isFromMe ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
                    <div className={`max-w-[70%] rounded-2xl px-5 py-3 shadow-md ${msg.isFromMe ? 'bg-brand-500 text-white rounded-tr-none' : 'bg-white text-gray-800 border border-gray-100 rounded-tl-none'}`}>
                      <p className="text-[15px] leading-relaxed whitespace-pre-wrap">{msg.text}</p>
                      <div className={`text-[10px] mt-2 text-right font-medium ${msg.isFromMe ? 'text-brand-100' : 'text-gray-400'}`}>
            ...
            <footer className="bg-[#f0f2f5] p-4 flex items-center z-10 border-t border-gray-200">
              <form onSubmit={handleSendMessage} className="flex-1 flex items-center bg-white rounded-xl overflow-hidden shadow-sm pl-4 pr-2 py-2 border border-gray-200 focus-within:ring-2 focus-within:ring-brand-500/20 focus-within:border-brand-500 transition-all">
                <input
                  ...
                  className="flex-1 bg-transparent border-none focus:outline-none text-[15px] text-gray-800 py-1"
                  autoComplete="off"
                />
                <button type="submit" disabled={!inputText.trim()} className="p-2.5 bg-brand-500 text-white rounded-lg hover:bg-brand-600 disabled:opacity-50 disabled:hover:bg-brand-500 transition-all ml-2 active:scale-95">
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center z-10 bg-white/50 backdrop-blur-sm m-4 rounded-3xl border border-white shadow-xl">
            ...
            <p className="text-gray-500 max-w-sm mb-6">Selecione um lead no menu lateral esquerdo ou clique no botão + para iniciar uma nova conversa.</p>
          </div>
        )}
      </div>
    </div>
